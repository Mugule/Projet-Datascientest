...
if selectedMenu == "Methodologie":
    ...
    with tabn:
        ...
        fig_ST = px.scatter(df_merged, x='Date', y='Sentiment_NLTK', color='Game Name', size='Clean_Tweet_Text', 
                         color_discrete_sequence=colorsTop5, title='Sentiment vs Tweets par jour', category_orders={'Game Name': game_order})
        ...
        st.markdown(">Super Smash Bros Ultimate, Pokémon et Zelda Breath of the Wild. Trois jeux qui engagent, à priori, davantage les joueurs présents sur Twitter et de façon continue que les autres.")
        st.markdown(">**Mario Kart 8 Deluxe :**")
        st.markdown(">La corrélation entre les sentiments et le nombre n’est pas très puissante et s’opère de façon discontinue. L’engagement des joueurs sur le temps n’est pas homogène. Peut-être un lien avec la sortie du jeu et la sortie de dlc par exemple…")
        st.markdown(">**Animal Crossing : New Horizon :**")
        st.markdown(">La relation de corrélation entre les sentiments et le nombre de tweets est forte les 3 premières années de ventes même si cela ne dépasse pas une moyenne de sentiment entre 0 et 0.5. Il y a une très nette rupture à partir de 2022 avec très peu de points visibles.")
    ...
    with tabt:
        ...
        game_name_mapping = {
            'Animal Crossing : New Horizon': 'ACNH',
        ...
        fig = px.line(df_tous_jeux, x="tweets_par_jour", y="nombre_de_tweets",
                      color="nom_jeu", color_discrete_sequence=colorsTop5,
    ...
    # %%%% Scrapping
        ...
        if dfScrap_x > 0:
            ...
            with colL:
                ...
                figUser.update_layout(showlegend=False)
